/**
 * ============================================================================
 *  ⚠️  OWASP A04:2025 — CRYPTOGRAPHIC FAILURES  ⚠️
 * ============================================================================
 ...
 *   GET  /api/a04/weak-hash-users
 *   POST /api/a04/crack-hash { hash }
 ...
 *            Passwords — CWE-328
 *   GET /api/a04/legacy-users
 *   GET /api/a04/legacy-users/:id/decrypt
 ...
 *            — CWE-327/CWE-326
 *   POST /api/a04/ecb-demo { pin1, pin2 }
 ...
 *   POST /api/a04/reset/request { email }
 *   POST /api/a04/reset/verify  { email, token }
 ...
 *            Requests — CWE-295
 *   GET /api/a04/tls-check?target=expired|self-signed|wrong-host|untrusted-root
 ...
 *   POST /api/a04/legacy-login { username, password }
 *   GET  /api/a04/logs
...
router.post('/magic-link', (req, res) => {
  ...
  const magicLink = `/api/a04/magic-login?token=${token}&email=${encodeURIComponent(email)}`;
  ...
  res.json({ magicLink, note: 'This link (with the token in cleartext) is what got written to the access log — check GET /api/a04/logs.' });
});
